...
const serializeOutput = (output: SpentOutput): Buffer => (
    Buffer.concat([int64LE(output.value), serializeScript(output.script)])
);

/**
 * Implements the Taproot key-path form of Bitcoin Knots PR #357's unified
 * signature hash. This intentionally supports only SIGHASH_ALL|UNIFIED: the
 * split primitive does not need the other modes and rejecting them avoids an
 * accidental divergence from the fork's consensus rules.
 */
export function hashForUnifiedKeypath(
    ...
    if (!Number.isSafeInteger(inputIndex) || inputIndex < 0 || inputIndex >= transaction.ins.length) {
        throw new Error('Unified sighash input index is out of range');
    }
    if (spentOutputs.length !== transaction.ins.length) {
        throw new Error('Unified sighash requires the spent output for every input');
    }

    const prevouts = Buffer.concat(transaction.ins.map(input => (
        Buffer.concat([Buffer.from(input.hash), uint32LE(input.index)])
    )));
    ...
    const outputs = Buffer.concat(transaction.outs.map(output => serializeOutput({
        value: output.value,
        script: output.script
    })));

    const message = Buffer.concat([
        Buffer.from([0x00, hashType]),
        uint32LE(transaction.version),
        uint32LE(transaction.locktime),
        Buffer.from([0x00]), // reserved fifth locktime byte
        sha256(prevouts),
        sha256(amounts),
        sha256(scripts),
        sha256(sequences),
        sha256(outputs),
        Buffer.from([0x02]), // Taproot key-path script type
        uint32LE(inputIndex),
        Buffer.from([0x00]) // no annex
    ]);
